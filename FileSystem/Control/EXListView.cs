...
namespace EXControls
{
        ...
        private void this_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            else if (col.GetType() == typeof(EXBoolColumnHeader))
            {
                ...
                if (boolcol.Editable)
                {
                    ...
                    boolsubitem.OnValueChanged(lstvItem, boolsubitem, _col, boolsubitem.BoolValue);
                }
            }
        }
    ...
    public delegate void EXBoolListViewSubItemEventHandler(ListViewItem item, EXBoolListViewSubItem subItem, int colIndex,bool value);
    public class EXBoolListViewSubItem : EXListViewSubItemAB
    {
        public event EXBoolListViewSubItemEventHandler ValueChanged;

        public void OnValueChanged(ListViewItem item, EXBoolListViewSubItem subItem, int colIndex,bool value)
        {
            if (ValueChanged != null)
                ValueChanged(item, subItem,colIndex, value);
        }
        ...
        public override int DoDraw(DrawListViewSubItemEventArgs e, int x, EXColumnHeader ch)
        {
            ...
            int imgx = e.Bounds.X + ((int)(e.Bounds.Width / 2)) - ((int)(boolimg.Width / 2));
            e.Graphics.DrawImage(boolimg, imgx, imgy, boolimg.Width, boolimg.Height);
            x += boolimg.Width + 2 + imgx;
            return x;
        }
